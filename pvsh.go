...
func Encode(id mcl.Fr, pk mcl.G2, sh mcl.Fr, g2 mcl.G2) (string, error) {
	// Algorithm 1
	r := mcl.Fr{}
	r.SetByCSPRNG()
	...
	// Algorithm 2
	var Q mcl.G1
	if err := Q.HashAndMapTo(appendBuffer(id.Serialize(), pk.Serialize())); err != nil {
		return "", err
	}
...
func Verify(id mcl.Fr, pk mcl.G2, ph mcl.G2, esh string, g2 mcl.G2) error {
...
func Decode(id mcl.Fr, pk mcl.G2, sk mcl.Fr, esh string) (*mcl.Fr, error) {
	eshArray := strings.Split(esh, ".")
	if len(eshArray) != 3 {
